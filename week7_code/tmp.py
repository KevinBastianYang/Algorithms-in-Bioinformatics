import sys
import re
"""readin(): readin the parameters and process"""
def readin():
	parameters = sys.argv
	error = 0
	if len(parameters) == 1:
...
"""transte(p_pattern): transit the prosite pattern to regular expression"""
def transit(p_pattern):
	amino_acid = "ACDEFGHIKLMNPQRSTVWY"
	"""split the pattern into individual ones by '-' """
	p_pattern = p_pattern.replace('\\','').strip().split('-')
	"""r_pattern is the regex pattern that is converted from individual ones"""
	r_pattern = ""
	"""prefix is the corresonding information of match like start\end\name\description"""
	...
	"""process each individual pattern"""
